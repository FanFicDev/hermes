...
class HtmlView:
    def __init__(
        self, html: str, markdown: bool = True, extraTitles: Optional[List[str]] = None
    ) -> None:
        hrRes = [
    ...
    def __addLine(self, line: str) -> None:
        ...
        while (mhr.startswith("<strong>") and mhr.endswith("</strong>")) or (
            mhr.startswith("<em>") and mhr.endswith("</em>")
        ):
        ...
        if (
            ...
            for squiggleBreak in squiggleBreaks:
                if sq == squiggleBreak:
                    line = "<hr />"
                    break
        if (len(line) - len(line.strip("=")) > 8) and (
            line.strip("=") == "HG/MM" or line.strip("=") == "MM/HG"
        ):
            line = "<hr />"
        ...
        if len(line) > (80 * 60 * 1000000):  # TODO
            raise Exception(f"error: extremely long line: {len(line)}\n{line}")
...
class ChapterView:
    def __init__(
        ...
        if header == True:
            ...
            if chapter.title is not None and len(chapter.title) > 0:
                self.text += [["", f"Chapter {chapter.chapterId}: {chapter.title}", ""]]
    ...
    def __wrapLine(self, idx: int) -> None:
        ...
        if isinstance(line, list):
            ...
            self.wtext[idx] = util.wrapText(util.equiPad(line, self.width), self.width)
            return
        self.wtext[idx] = util.wrapText(self.preWrap + line + self.postWrap, self.width)
...
class StoryView(Widget):
    ...
    def handleKey(self, key: int) -> bool:
        ...
        if key == ord("c"):
            ...
            self.currentLineColor = self.currentLineColorOptions[
                self.currentLineColorIdx
            ]
            return True
        if key == ord("i"):
