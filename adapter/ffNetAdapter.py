...
class FFNAdapter(Adapter):
    def __init__(self) -> None:
        super().__init__(
            True, "https://www.fanfiction.net", "fanfiction.net", FicType.ff_net, "ffn"
        )
    ...
    def handleCrossoverFandom(
        ...
        if len(missingMessy) > 0:
            util.logMessage(f"unknown messy fandom: {missingMessy} from {href}")
            return fandoms

        # check crossover value
        expected = f"{messys[0]}_and_{messys[1]}_Crossovers"
        if expected != fandom:
            util.logMessage(f'crossover got "{fandom}" expected "{expected}"')
            return fandoms
    ...
    def parseInfoInto(self, fic: Fic, wwwHtml: str) -> Fic:
        ...
        for a in preStoryLinksLinks:
            ...
            if (
                len(hrefParts) == 5
                and hrefParts[1].endswith("_Crossovers")
                and len(hrefParts[0]) == 0
                and len(hrefParts[4]) == 0
            ):
