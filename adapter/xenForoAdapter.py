...
class XenForoAdapter(Adapter):
    ...
    def readerSoftScrape(self, fic: Fic) -> None:
        ...
        util.logMessage(f"readerSoftScrape|fic.id: {fic.id}|len(urls): {len(urls)}")
        for url in urls:
            self.scrapeLike(url)
    ...
    def parseInfoInto(self, fic: Fic, wwwHtml: str) -> Fic:
        ...
        try:
            ...
            for threadmark in threadmarks:
                if (
                    threadmark.find("span", {"class": "message-newIndicator"})
                    is not None
                ):
                    continue
                a = threadmark.find("a")
            ...
            if postId in postSoups and postSoups[postId] is not None:
                ...
                if postId is not None:
                    ...
                    if len(poss) != 1:
                        ...
                        poss = pageSoup.find_all(
                            self.postContainer, {"id": "js-" + postId}
                        )
                    if len(poss) != 1:
    ...
    def cleanTitle(self, title: str) -> Tuple[str, Set[str], Set[str]]:
        ...
        fandomFragments: List[Tuple[str, List[str], List[str]]] = [
            ...
            (
                "Worm alt power/crossover Hellsing Ultimate",
                ["Worm", "Hellsing"],
                ["altpower"],
            ),
            ("Worm Altpower Fic", ["Worm"], ["altpower"]),
            (
                "Worm | AltPower | Simurgh!Taylor",
                ["Worm"],
