    ...
    def tryParseUrl(self, url: str) -> Optional[FicId]:
        ...
        if url.find("/chapters/") >= 0 and url.find("/works/") < 0:
            ...
            for a in soup.find_all("a"):
                if a.get_text() == "Entire Work":
                    return self.tryParseUrl(
                        self.baseUrl + a.get("href")[len("/works/") :]
                    )
            else:
        ...
        if len(pieces) >= 3 and pieces[1] == "chapters" and pieces[2].isnumeric():
            ...
            mchaps = FicChapter.select(
                {"ficId": fic.id, "localChapterId": localChapterId}
            )
            if len(mchaps) == 1:
        ...
        if fic.chapterCount > 1:
            ...
            for cid in range(1, fic.chapterCount + 1):
                ...
                chap.url = "{}{}/chapters/{}?view_adult=true".format(
                    self.baseUrl,
                    fic.localId,
                    localChapterIdSelect[cid - 1].get("value"),
                )
        ...
        if fandomDd is not None:
            ...
            for ft in fandomTags:
                ...
                if (
                    ...
                    f == "iron man (movies)"
                    or f == "iron man - all media types"
                    or f == "iron man (comic)"
                    or f == "iron man - fandom"
                    ...
                    f == "x-men - all media types"
                    or f == "x-men (movieverse)"
