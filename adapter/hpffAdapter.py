...
class HarryPotterFanfictionAdapter(Adapter):
    ...
    def getArchiveChapterInfo(self, chapterId: int) -> Optional[List[str]]:
        ...
        if len(matching) < 1:
            return None
        if len(matching) > 1:
            raise Exception(f"chapterId is in map multiple times: {chapterId}")
        return matching[0]
    ...
    def parseInfoInto(self, fic: Fic, wwwHtml: str) -> Fic:
        ...
        for a in soup.findAll("a"):
            href = a.get("href")
            if not href.startswith(disclaimerJs) and href != f"?psid={fic.localId}":
                continue
            fic.title = a.getText()
            break
        else:
